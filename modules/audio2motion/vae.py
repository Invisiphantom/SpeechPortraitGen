        ...
        self.pre_net = nn.Sequential(
            *[
                (
                    ...
                    if i == 0
                    else nn.Conv1d(hidden_channels, hidden_channels, kernel_size=s * 2, stride=s, padding=s // 2)
                )
        ...
        self.pre_net = nn.Sequential(
            *[
                (
                    ...
                    if i == 0
                    else nn.ConvTranspose1d(hidden_channels, hidden_channels, kernel_size=s, stride=s)
                )
                for i, s in enumerate(strides)
            ]
        )
...
class FVAE(nn.Module):
    def __init__(
        ...
        self.encoder = FVAEEncoder(
            in_out_channels, hidden_channels, latent_size, kernel_size, enc_n_layers, gin_channels, strides=strides
        )
        if use_prior_glow:
        ...
        if sqz_prior:
            ...
            if self.in_out_channels in [7, 64]:
                ...
                self.exp_decoder = FVAEDecoder(
                    hidden_channels, hidden_channels, 64, kernel_size, dec_n_layers, gin_channels, strides=strides
                )
                self.pose_decoder = FVAEDecoder(
                    hidden_channels, hidden_channels, 7, kernel_size, dec_n_layers, gin_channels, strides=strides
                )
            ...
            self.decoder = FVAEDecoder(
                latent_size, hidden_channels, in_out_channels, kernel_size, dec_n_layers, gin_channels, strides=strides
            )

        self.prior_dist = dist.Normal(0, 1)

    def forward(self, x=None, x_mask=None, g=None, infer=False, temperature=1.0, **kwargs):
        """
        ...
        if not infer:
            ...
            if self.sqz_prior:
                ...
                if self.in_out_channels == 71:
                    x_recon = torch.cat(
                        [self.exp_decoder(style_encoding, x_mask, g), self.pose_decoder(style_encoding, x_mask, g)],
                        dim=1,
                    )
                else:
            ...
            if self.sqz_prior:
                ...
                if self.in_out_channels == 71:
                    x_recon = torch.cat(
                        [self.exp_decoder(style_encoding, 1, g), self.pose_decoder(style_encoding, 1, g)], dim=1
                    )
                else:
                    x_recon = self.decoder(style_encoding, 1, g)
            else:
...
class VAEModel(nn.Module):
    def __init__(self, in_out_dim=64, audio_in_dim=1024, sqz_prior=False, cond_drop=False, use_prior_flow=True):
        ...
        self.downsampler = LambdaLayer(
            lambda x: F.interpolate(x.transpose(1, 2), scale_factor=0.5, mode="linear").transpose(1, 2)
        )
...
class PitchContourVAEModel(nn.Module):
    def __init__(
        self, hparams, in_out_dim=64, audio_in_dim=1024, sqz_prior=False, cond_drop=False, use_prior_flow=True
    ):
        super().__init__()
