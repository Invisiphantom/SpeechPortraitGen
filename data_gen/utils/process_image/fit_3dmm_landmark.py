...
face_model = ParametricFaceModel(
    bfm_folder="deep_3drecon/BFM", camera_distance=10, focal=1015, keypoint_mode="mediapipe"
)
face_model.to("cuda")
...
def cal_lan_loss_mp(proj_lan, gt_lan):
    ...
    eye = [33, 246, 161, 160, 159, 158, 157, 173, 133, 155, 154, 153, 145, 144, 163, 7] + [
        263,
        466,
        388,
        387,
...
def fit_3dmm_for_a_image(img_name, debug=False, keypoint_mode="mediapipe", device="cuda:0", save=True):
    ...
    assert lms is not None, "无法获得2d关键点, 请确保源图片有头"

    lms = lms[:468].reshape([468, 2])
    lms = torch.FloatTensor(lms).to(device)
    lms[:, 1] = img_h - lms[:, 1]  # 翻转y轴
    ...
    for i in range(int((num_frames - 1) / batch_size + 1)):
        ...
        for j in range(50):
            proj_geo = face_model.compute_for_landmark_fit(sel_id_para, sel_exp_para, sel_euler_angle, sel_trans)
            loss_lan = cal_lan_loss_fn(proj_geo[:, :, :2], lms.unsqueeze(0).detach())
        ...
        print(
            f"batch {i} | loss_lan: {loss_lan.item():.2f}, loss_reg_id: {loss_regid.item():.2f},loss_reg_exp: {loss_regexp.item():.2f}"
        )
    ...
    if debug:
        ...
        debug_name = (
            img_name.replace("/images_512/", "/coeff_fit_mp_debug/")
            .replace(".png", "_debug.png")
            .replace(".jpg", "_debug.jpg")
        )
        try:
...
if __name__ == "__main__":
    ...
    face_model = ParametricFaceModel(
        bfm_folder="deep_3drecon/BFM", camera_distance=10, focal=1015, keypoint_mode=args.keypoint_mode
    )
