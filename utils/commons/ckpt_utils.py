...
def load_ckpt(cur_model, ckpt_base_path:str, model_name='model', force=True, strict=True, steps=None, verbose=True):
    if os.path.isfile(ckpt_base_path):
        base_dir = os.path.dirname(ckpt_base_path)
        ckpt_path = ckpt_base_path
        checkpoint = torch.load(ckpt_base_path, map_location='cpu')
    else: # 如果是目录, 则加载最新的ckpt
        base_dir = ckpt_base_path
        checkpoint, ckpt_path = get_last_checkpoint(ckpt_base_path, steps)
    if checkpoint is not None:
        state_dict = checkpoint["state_dict"]
        if len([k for k in state_dict.keys() if '.' in k]) > 0: # 去除掉state_dict中的model_name
        ...
        print(f"| 从{ckpt_path}中加载模型, strict={strict}")
    else:
        e_msg = f"| 在目录{base_dir}未找到ckpt文件"
        if force:
            assert False, e_msg
        else:
            print(e_msg)
